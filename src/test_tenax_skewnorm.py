...
name_col = "ppt" #name of column containing data to extract
...
t_data.set_index('valid_time', inplace=True)
...
blocks_id = dict_ordinary["60"]["year"].to_numpy()  # Replace with your actual data
...
RL, _, P_check = S.model_inversion(F_phat, g_phat, n, Ts, temp_method="skewnorm") 
print(RL)
elapsed_time = time.time() - start_time
# Print the elapsed time
...
TNX_FIG_valid(AMS,S.return_period,RL,smev_RL,RL_unc,smev_RL_unc, ylimits=[0,200] )
plt.title('fig 4')
plt.ylabel('10-minute precipitation (mm)')
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15))
plt.show()
